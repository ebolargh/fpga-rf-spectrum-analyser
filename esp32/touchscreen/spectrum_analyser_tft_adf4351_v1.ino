...
// Shared SPI wiring is fixed by the existing touchscreen installation.
// TFT and ADF4351 share SCK GPIO 18 and MOSI GPIO 23.
// The ADF4351 has its own LE pin so it cannot select the TFT accidentally.
...
constexpr uint32_t ADF4351_INITIAL_HZ = 49500000UL;

uint32_t pllFrequencyHz = ADF4351_INITIAL_HZ;
bool pllReady = false;
...
void deselectOtherSpiDevices() {
  digitalWrite(TFT_CS_PIN, HIGH);
  digitalWrite(TOUCH_CS_PIN, HIGH);
}

bool setPllFrequency(uint32_t frequencyHz) {
  if (!pllReady) {
    Serial.println("ADF4351 frequency change ignored: PLL not ready");
    return false;
  }

  deselectOtherSpiDevices();

  int result = vfo.setf(frequencyHz);

  if (result == 0) {
    pllFrequencyHz = frequencyHz;
    Serial.print("ADF4351 frequency set to ");
    Serial.print(pllFrequencyHz);
    Serial.println(" Hz");
    return true;
  }

  Serial.print("ADF4351 setf() failed, code ");
...
bool initialisePll() {
  ...
  if (vfo.setrf(ADF4351_REFERENCE_HZ) != 0) {
    Serial.println("ADF4351 reference setup failed");
    return false;
  }
  ...
  pllReady = true;
  return setPllFrequency(ADF4351_INITIAL_HZ);
}
...
void handleCentreTouch(uint16_t x, uint16_t y) {
  if (hitButton(x, y, 15, 105, 140, 48)) {
    ...
    // Apply centre-frequency changes only when DONE is pressed.
    // This avoids repeatedly reprogramming the PLL for every button touch.
    if (!setPllFrequency(centreFrequency)) {
      Serial.println("Centre frequency was not applied to the ADF4351");
    }
