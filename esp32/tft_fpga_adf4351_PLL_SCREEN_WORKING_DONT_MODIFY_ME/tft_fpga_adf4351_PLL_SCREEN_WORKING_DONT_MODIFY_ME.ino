...
//
// Combined test:
// - TFT/touch remain on SCLK 18, MISO 19, MOSI 23
// - ADF4351 uses CLK 32, DATA 33, LE 27, CE 25
// - FPGA UART RX remains GPIO 34
...
// Proven pin assignments. The PLL has its own physical clock/data wires.
...
bool initialisePllAtTestFrequency() {
  ...
  // Temporarily move the global SPI controller from the TFT pins to the
  // separately wired PLL pins. The TFT and touch chip-selects remain HIGH.
  SPI.end();
  SPI.begin(
    ADF4351_CLK_PIN,
    -1,
  ...
  Serial.println(" Hz");
  Serial.println("PLL setup complete on CLK 32 / DATA 33");
  return true;
}
...
void setup() {
  ...
  // UART2 is initialized after TFT and PLL setup in this diagnostic.
  // This ensures GPIO 34 is assigned to UART RX last.
  ...
  // Program the PLL on its separate GPIO 32/33 wires.
  bool pllStarted = initialisePllAtTestFrequency();

  // Return the global SPI controller to the unchanged TFT/touch wiring.
  SPI.end();
  SPI.begin(
    TFT_SCLK_PIN,
    TFT_MISO_PIN,
  ...
  Serial.print("FPGA UART2 initialized last on GPIO ");
